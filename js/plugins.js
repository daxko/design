...
(function(root, factory) {
  ...
  var globalObject = typeof global !== 'undefined' ? global : this || window,
    ...
    supportTransitions =
      Webkit + Transition in doc[style] ||
      Transition[toLowerCase]() in doc[style],
    transitionEndEvent =
      Webkit + Transition in doc[style]
      ...
      for (; element && element !== document; element = element[parentNode]) {
        ...
        if (firstChar === '.') {
          ...
          if (
            ...
            hasClass(element, selector.replace('.', ''))
          ) {
            return element;
          }
        } else if (firstChar === '#') {
    ...
    styleTip = function(link, element, position, parent) {
      ...
      if (position === top) {
        ...
        element[style][top] =
          ...
          'px'; // isPopover is trying to fix bug with V4beta CSS
        element[style][left] =
          rect[left] +
          scroll.x -
    ...
    var self = this,
      ...
      clickHandler = function(e) {
        ...
        if (
          eventTarget === element ||
          eventTarget === parent ||
          eventTarget[parentNode] === element
        ) {
          ...
          if (
            eventTarget === menu ||
            (menu.contains(eventTarget) && (self.persist || hasData))
          ) {
            return;
          } else {
            relatedTarget = null;
        ...
        (/\#$/.test(eventTarget.href) ||
          (eventTarget[parentNode] &&
            /\#$/.test(eventTarget[parentNode].href))) &&
          e.preventDefault(); // should be here to prevent jumps
      },
      // private methods
      show = function() {
